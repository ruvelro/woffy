...
- `Confirmed`: `db_init` creates and migrates the local SQLite schema idempotently.
- `Confirmed`: Woffu credentials and tokens are scoped by worker email.
- `Confirmed`: `run due` queries active schedules for the current weekday and minute, then uses `run_guard` to avoid duplicate execution.
