...
sed 's/^VERSION="3.1.1"/VERSION="2.0.0"/' "$repo_dir/woffy.sh" >"$smoke_bin/woffy"
...
[ "$(woffy version)" = "woffy v3.1.1" ]
[ -f "$smoke_bin/woffy.previous" ]
[ "$(sqlite3 "$HOME/.woffy/woffy.db" 'PRAGMA user_version;')" = "4" ]
woffy doctor --json | jq -e '.version=="3.1.1" and .schema_version==4 and .journal_mode=="wal"' >/dev/null
